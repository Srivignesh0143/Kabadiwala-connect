...
export default function RegisterPage() {
  ...
  const handleMaterialChange = (material) => {
    setForm((prev) => {
      const alreadySelected =
        prev.materialsAccepted.includes(material);

      return {
        ...prev,
        materialsAccepted: alreadySelected
          ? prev.materialsAccepted.filter(
              (item) => item !== material
            )
          : [...prev.materialsAccepted, material],
      };
    });
  };
  ...
  const handleSubmit = async (event) => {
    ...
    // Recycler must select at least one material
    if (
      form.role === 'RECYCLER' &&
      form.materialsAccepted.length === 0
    ) {
      toast.error(
        'Please select at least one material you can recycle'
      );
      return;
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-100 px-4 py-10 text-slate-900">
      <div className="w-full max-w-xl rounded-3xl border border-slate-200 bg-white p-8 shadow-[0_20px_45px_rgba(15,23,42,0.08)]">
        ...
        >
          ...
          {form.role === 'RECYCLER' && (
            <div className="md:col-span-2 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
              ...
              <p className="mb-4 text-sm text-slate-600">
                Select all the e-waste materials your recycling facility can process.
              </p>
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                ...
                {MATERIAL_TYPES.map((material) => (
                  ...
                  <label
                    key={material}
                    className="flex cursor-pointer items-center gap-2 rounded-lg bg-white p-2 text-sm text-slate-700 shadow-sm"
                  >
          ...
          <button
            type="submit"
            className="md:col-span-2 w-full rounded-xl bg-emerald-600 px-4 py-3 font-semibold text-white transition hover:bg-emerald-500"
          >
            Register
          </button>
